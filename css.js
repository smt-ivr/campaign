...
.lang-btn.active { background: var(--dark-blue); color: white; box-shadow: 0 2px 5px rgba(0,0,0,0.15); }
.lang-btn:hover:not(.active) { color: var(--dark-blue); }

/* ======== פריסת עמוד - סיידבר המלצה ======== */
.content-layout {
    display: flex;
    gap: 15px;
...
.rec-img {
    ...
    height: auto;
    border-radius: 8px;
    pointer-events: none; /* הגנה מפני גרירה ולחיצה ימנית */
    user-select: none;
...
.info-content { display: flex; flex-direction: column; flex-grow: 1; }
...
.currency-badge { padding: 5px 14px; border-radius: 20px; font-size: 0.95rem; font-weight: 700; display: flex; align-items: center; gap: 4px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); backdrop-filter: blur(5px); }
...
.mini-leaderboard { display: flex; flex-direction: column; flex-grow: 1; position: relative; overflow: hidden; }
.mini-leaderboard h3 { font-size: 1.05rem; border-bottom: 1px solid rgba(255,255,255,0.2); padding-bottom: 8px; margin: 0; text-align: center; height: 32px; }

.scroll-list { position: absolute; top: 40px; bottom: 0; left: 0; right: 0; overflow-y: auto; padding-right: 5px; }
...
.sol-percent { font-size: 0.8rem; font-weight: bold; min-width: 55px; width: auto; color: var(--gold); text-align: left; direction: ltr; flex-shrink: 0; white-space: nowrap; }

/* ======== פאנל תרומה: קומפקטי ומהודק ======== */
.donate-panel {
    width: 68%; 
    padding: 15px 20px; 
    display: flex; flex-direction: column; justify-content: space-between; text-align: center;
}
.donate-panel h2 { margin: 0 0 8px 0; font-size: 1.25rem; color: var(--dark-blue); font-weight: 800; }

.unified-amount-wrapper {
    display: flex; align-items: center; justify-content: space-between;
    background: #ffffff; border: 2px solid #e2e8f0; border-radius: 10px;
    padding: 4px 8px; margin-bottom: 10px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.03); 
    transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); position: relative;
}
.unified-amount-wrapper:focus-within { border-color: var(--dark-blue); box-shadow: 0 0 0 3px rgba(26, 54, 93, 0.1), 0 4px 10px rgba(0, 0, 0, 0.05); transform: translateY(-1px); }

.hero-amount-input {
    flex-grow: 1; width: 100%; padding: 6px 8px; font-size: 1.4rem; font-weight: 800; color: var(--dark-blue);
    border: none; background: transparent; text-align: center; outline: none; font-family: inherit; letter-spacing: 0.5px;
}
...
.currency-toggle-inline { display: flex; background: #f1f5f9; border-radius: 8px; padding: 3px; gap: 2px; flex-shrink: 0; }
.curr-btn { display: flex; align-items: center; justify-content: center; min-width: 38px; height: 32px; font-size: 1.1rem; font-weight: 800; cursor: pointer; border: none; background: transparent; color: #94a3b8; border-radius: 6px; transition: all 0.25s ease; font-family: inherit; }
...
.compact-form { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; margin-bottom: 8px; }
...
.compact-form input, .compact-form select { width: 100%; padding: 6px; border: 1px solid var(--border); border-radius: 6px; font-size: 0.85rem; font-family: inherit; background: #f8fafc; outline: none; transition: 0.2s; text-align: center; }
...
/* חלון סליקה רספונסיבי */
.payment-area { width: 100%; min-height: 250px; flex-shrink: 0; position: relative; display: flex; flex-direction: column; justify-content: center; align-items: center; overflow: hidden; transition: height 0.3s ease; }
...
#NedarimFrame { width: 100%; height: 100%; min-height: 250px; border: none; position: relative; z-index: 2; background: transparent; }

.elegant-submit { width: 100%; padding: 8px; background: var(--dark-blue); color: white; border: none; border-radius: 8px; font-size: 1.1rem; font-weight: 700; font-family: inherit; cursor: pointer; transition: 0.3s; margin-top: 4px; text-align: center; }
...
[dir="ltr"] .top-controls { flex-direction: row-reverse; }

/* ======== התאמות רספונסיביות מלאות לטלפונים ======== */
@media (max-width: 900px) {
    ...
    .content-layout {
        ...
        border-radius: var(--radius);
        box-shadow: 0 15px 35px rgba(0,0,0,0.1);
        overflow: hidden; /* שומר על פינות עגולות חלקות לכל הבלוק */
        margin-top: 0;
    }
    .main-card { display: contents; } /* מבטל את הקופסא כדי לאפשר סידור מחדש של האלמנטים */
    ...
    /* תמונת המלצה מקבלת רקע כחול ומשתלבת ברצף המובייל בדיוק באמצע! */
    .side-content { order: 3; width: 100%; margin: 0; }
    .recommendation-box {
        background: var(--dark-blue);
        border: none;
        border-radius: 0;
    ...
    .rec-title { color: var(--gold); margin: 0; font-size: 1.1rem; }
    .rec-img { max-height: 100px; width: auto; box-shadow: 0 4px 10px rgba(0,0,0,0.2); }
    ...
    .donate-panel { width: 100%; order: 5; padding: 20px 15px; background: var(--card-bg); display: flex; }
